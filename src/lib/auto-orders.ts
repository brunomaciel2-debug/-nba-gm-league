...
// no GM, doesn't matter: a rotation stays the same until someone actually
// changes it), and only falls back to a fresh generic usage-sorted lineup
// for a team that has genuinely never had any order at all. Shared by the
// admin manual-trigger route AND runWeeklySimulation itself — used to be
// a route nobody actually called automatically, so a team nobody visited
// the Orders page for (most often a team with no active GM) silently kept
// simulating on a default 'motion'/'man'/'normal' order, forever, no
// matter how many real weeks had already passed.
export async function ensureWeeklyOrders(week: number): Promise<{ generated: number, carriedForward: number, errors: string[] }> {
  ...
  return { generated, carriedForward, errors }
}
