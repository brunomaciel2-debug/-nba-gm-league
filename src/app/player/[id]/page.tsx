...
const ATTR_TIPS: Record<string,string> = {
  usage:       "Usage Rate â how often this player is involved in offensive plays. High usage = primary option. Impacts scoring consistency and team dependency.",
  three:       "Three Point â shooting ability from beyond the arc. Affects three-point percentage and floor spacing.",
  layup:       "Layup â finishing ability at the rim. Impacts shot conversion near the basket.",
  dunk:        "Dunk â ability to finish with power above the rim. High value reduces blocks against near-basket shots.",
  mid:         "Mid-Range â ability to score from mid-range. Affects pull-up jumpers and post fades.",
  ft:          "Free Throws â free throw shooting accuracy. Critical in late-game situations and when fouled.",
  siq:         "Shot IQ â decision-making on shot selection. High value = player takes good shots and avoids contested ones.",
  draw_foul:   "Draw Foul â ability to get to the free throw line. High value earns more foul calls on drives and in the post.",
  blk:         "Block â ability to block opponent shhots. Impacts interior defence and psychological deterrence.",
  stl:         "Steal â ability to strip the ball or intercept passes. Affects transition offence and opponent turnover rate.",
  idef:        "Interior Defense â ability to defend in the paint against post players and drivers. Reduces easy buckets inside.",
  pdef:        "Perimeter Defense â ability to guard on the perimeter. Affects opponent shooting percentage on the wing.",
  def_reb:     "Defensive Rebound â ability to secure rebounds after opponent misses. Limits second-chance points.",
  off_reb:     "Offensive Rebound â ability to recover missed shots offensively. Creates extra possessions for the team.",
  stamina:     "Stamina â endurance across a game. Low stamina = significant stat drop in the 4th quarter and back-to-backs.",
  durability:  "Durability â resistance to injuries. Low durability increases injury probability during games and practice.",
  ball_hdl:    "Ball Handling â ability to dribble under pressure and create off the dribble. Affects turnover rate.",
  pass_vis:    "Pass Vision â ability to read the defence and find open teammates. Impacts assist opportunities.",
  pass_iq:     "Pass IQ â decision-making when passing. High value = fewer turnovers, better offensive flow.",
  assist_role: "Assist Role â how naturally this player fits into a pass-first role. High value = better assist consistency.",
  pressure:    "Clutch/Pressure â performance in high-pressure moments (last 2 minutes, playoffs). Key for closers.",
  consistency: "Consistency â game-to-game variance in performance. High value = reliable production every night.",
  crowd_effect:"Crowd Effect â how much crowd noise affects this player. Low value = struggles in hostile environments.",
  streaky:     "Streaky â tendency to have hot and cold streaks. Can be explosive but unpredictable.",
  trash_talk:  "Trash Talk â ability to get in opponents' heads. High value slightly reduces opponent performance.",
  moral:       "Morale â current mental state. Affects consistency and willingness to perform. Impacted by playing time and results.",
}
...
export default async function PlayerPage({ params }: { params: { id: string } }) {
  ...
  const capFmt = (n:number) => n ? '$'+(n/1000000).toFixed(2)+'M' : 'â'
  const pct    = (m:number,a:number) => a>0?(m/a*100).toFixed(1)+'%':'â'
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{ color:tc, letterSpacing:'1px' }}>
                  {p.world_team_id && p.world_teams ? `${p.world_teams.name} Â· ${p.world_teams.country}` : team?.name} Â· {p.pos}
                </div>
      ...
      {p.nba_recruitable === false && (
        <div className="mb-4 px-4 py-3 rounded-xl flex items-center gap-2"
          ...
          <span className="text-xs font-semibold" style={{color:'#b45309'}}>
            International player â not available for NBA contracts. Pre-season friendly only.
          </span>
        </div>
      )}
          ...
          {(stats||[]).length > 0 ? (
            <div className="rounded-xl overflow-hidden mb-6" style={{ border:'1px solid #d4cdc5' }}>
              ...
              <table className="w-full text-xs" style={{minWidth:700}}>
                ...
                <tbody>
                  {(stats||[]).map((s:any,i:number) => {
                    const gp  = s.games||0
                    const avg = (v:number) => gp>0 ? (v/gp).toFixed(1) : 'â'
                    const avgM= (v:number) => gp>0 ? (v/gp).toFixed(0) : 'â'
                    const pctS= (m:number,a:number) => a>0 ? (m/a*100).toFixed(1)+'%' : 'â'
                    const oreb = s.oreb || 0
              ...
              <div className="px-4 py-2 text-xs" style={{color:'#a89f97',borderTop:'1px solid #e2dcd5',background:'#f5f1eb'}}>
                Per game averages Â· TD = Triple-Doubles Â· +/- = season total
              </div>
            </div>
          ) : (
            <div className="rounded-xl p-4 text-center mb-6" style={{ background:'#faf8f5', border:'1px solid #d4cdc5' }}>
              <p className="text-sm" style={{ color:'#5c554e' }}>No stats yet â season hasn't started.</p>
            </div>
          )}
                  ...
                  return (
                    <tr key={b.id} style={{background:i%2===0?'#faf8f5':'#f5f1eb',borderBottom:'1px solid #e2dcd5'}}>
                      <td className="px-2.5 py-2.5 whitespace-nowrap" style={{color:'#8a8279'}}>
                        {g.played_at ? new Date(g.played_at).toLocaleDateString('en-US',{month:'short',day:'numeric'}) : 'â'}
                      </td>
                      <td className="px-2.5 py-2.5 whitespace-nowrap">
                        <span style={{color:'#8a8279'}}>{isHome?'vs':'@'} </span>
                        <span style={{color:oppColor,fontWeight:600}}>{opp?.name||'â'}</span>
                      </td>
        ...
        <div className="rounded-xl overflow-hidden" style={{border:'1px solid #d4cdc5'}}>
          {(injuries||[]).length === 0 ? (
            <div className="px-4 py-5 text-center" style={{background:'#faf8f5'}}>
              <p className="text-sm" style={{color:'#8a8279'}}>No injury history â clean bill of health.</p>
            </div>
                ...
                <div key={i} className="flex items-center gap-3 px-4 py-3"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-xs" style={{color:'#8a8279'}}>
                      {a.season} Â· {a.period?.replace('week_','Week ').replace('month_','Month ').replace('season','Full Season')}
                    </div>
                  </div>
                  {a.stats_context?.ppg && (
                    <div className="text-xs font-semibold" style={{color:'#5c554e'}}>
                      {a.stats_context.ppg} PPG
                      {a.stats_context.rpg && ` Â· ${a.stats_context.rpg} RPG`}
                      {a.stats_context.apg && ` Â· ${a.stats_context.apg} APG`}
                    </div>
                  )}
                </div>
              ))}
