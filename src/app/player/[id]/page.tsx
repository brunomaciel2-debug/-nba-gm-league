...
export default async function PlayerPage({ params }: { params: { id: string } }) {
  ...
  const totalValue = (contracts||[]).reduce((sum:number,c:any) => sum+(c.salary||0), 0)

  // Filter out box scores with missing game data
  const validGames = (lastGames||[]).filter((b:any) => b?.games?.id)
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              <div>
                <div className="text-xs font-bold uppercase tracking-widest mb-1" style={{color:tc,letterSpacing:'1px'}}>
                  {p.world_team_id && p.world_teams ? `${p.world_teams.name} · ${p.world_teams.country}` : (team?.name || 'Free Agent')} · {p.pos}
                </div>
      ...
      {!p.team_id && (
        <OfferButton playerId={p.id} isAssigned={!!p.on_gleague_assignment}/>
      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {(contracts||[]).map((c:any,i:number) => {
                  ...
                  return (
                    <tr key={c.id}
                        style={{background:isCurrent?tc+'11':i%2===0?'#faf8f5':'#f5f1eb',borderBottom:'1px solid #e2dcd5'}}>
                      ...
                      <td className="px-4 py-2.5 text-right">
                        {c.waived && <span className="text-xs font-semibold px-2 py-0.5 rounded"
