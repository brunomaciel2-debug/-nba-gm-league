...
export default function PreseasonPage() {
  ...
  useEffect(() => {
    if (myTeamId) {
      setMyGames(allGames.filter(g => g.home_team === myTeamId || g.away_team === myTeamId))
    }
  }, [myTeamId, allGames])
  ...
  // Check if a date is available for my team
  ...
  // Get available opponents for a selected date
  const getAvailableOpponents = (date: Date) => {
    const dateStr = isoDate(date)
    const prevStr = isoDate(addDays(date, -1))
    const nextStr = isoDate(addDays(date, 1))

    const busyTeams = new Set(
      allGames
        .filter(g => g.scheduled_date && ['pending','accepted','scheduled','final'].includes(g.status))
        .filter(g => [prevStr, dateStr, nextStr].includes(g.scheduled_date))
        .flatMap(g => [g.home_team, g.away_team])
    )

    return {
      nba: nbaTeams.filter(t => t.id !== myTeamId && !busyTeams.has(t.id)),
      world: worldTeams.filter(t => !busyTeams.has(t.id)),
    }
  }
  ...
  const handleSchedule = async () => {
    ...
    const homeType = isHome ? 'nba' : opponentType
    const awayType = isHome ? opponentType : 'nba'
    ...
    const { error } = await supabase.from('preseason_games').insert({
      ...
      status: isWorldGame ? 'scheduled' : 'pending',
      scheduled_date: isoDate(selectedDate),
      notes,
    })
    ...
    if (error) {
      ...
      setMsg(isWorldGame ? '✅ Game scheduled!' : '✅ Request sent! Waiting for opponent to accept.')
      await loadData()
  ...
  const handleRespond = async (gameId: string, accept: boolean) => {
    await supabase.from('preseason_games').update({
  ...
  const getTeamName = (id: string, type: string) => {
    if (type === 'world') return worldTeams.find(t => t.id === id)?.name || id
    return nbaTeams.find(t => t.id === id)?.name || id
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-3xl font-black mb-1" style={{color:'#1a1512'}}>🏀 Pre-Season</h1>
        <p className="text-sm" style={{color:'#8a8279'}}>Oct 2–17, 2025 · Max 5 games per team · No back-to-back games</p>
      </div>
      ...
      {myTeamId && (
        <div className="grid grid-cols-3 gap-3 mb-6">
          {[
            { label: 'Games Scheduled', value: myActiveGames.length, max: MAX_GAMES, color: '#1d4ed8' },
            { label: 'Remaining Slots', value: MAX_GAMES - myActiveGames.length, color: '#15803d' },
            { label: 'Pending Requests', value: myGames.filter(g=>g.status==='pending' && g.requested_by !== myTeamId).length, color: '#b45309' },
          ].map(s => (
      ...
      {myTeamId && canScheduleMore && (
        <button onClick={() => setShowModal(true)}
          ...
          + Schedule Pre-Season Game
        </button>
      )}
      {myTeamId && !canScheduleMore && (
        <div className="mb-6 px-4 py-3 rounded-xl text-sm font-semibold"
             style={{background:'#fee2e2',color:'#dc2626',border:'1px solid #fca5a5'}}>
          ⚠️ Maximum 5 games reached
        </div>
      )}

      {/* Pending requests for me to accept */}
      {myTeamId && (() => {
                ...
                <div key={g.id} className="flex items-center gap-4 px-4 py-3 rounded-xl"
                  ...
                  <div className="flex-1">
                    <div className="font-bold text-sm" style={{color:'#1a1512'}}>
                      {getTeamName(g.home_team, g.home_type)} vs {getTeamName(g.away_team, g.away_type)}
                    </div>
                    <div className="text-xs" style={{color:'#8a8279'}}>
                      {g.scheduled_date ? fmtDate(g.scheduled_date) : 'Date TBD'}
                      {g.notes && ` · "${g.notes}"`}
                    </div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => handleRespond(g.id, true)}
                      className="px-3 py-1.5 rounded-lg text-xs font-bold"
                      style={{background:'#15803d',color:'#fff'}}>Accept</button>
                    <button onClick={() => handleRespond(g.id, false)}
                      className="px-3 py-1.5 rounded-lg text-xs font-bold"
                      style={{background:'#dc2626',color:'#fff'}}>Decline</button>
                  </div>
                </div>
              ))}
            </div>
      ...
      {myTeamId && myActiveGames.filter(g => g.status !== 'pending' || g.requested_by === myTeamId).length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-bold uppercase tracking-widest mb-3" style={{color:'#5c554e'}}>
            My Games
          </h2>
          <div className="flex flex-col gap-2">
            {myGames
              ...
              .map(g => {
                ...
                return (
                  <div key={g.id} className="flex items-center gap-3 px-4 py-3 rounded-xl"
                    ...
                    <div className="flex-1">
                      <div className="font-bold text-sm" style={{color:'#1a1512'}}>
                        {getTeamName(g.home_team, g.home_type)}
                        <span style={{color:'#8a8279',fontWeight:400}}> vs </span>
                        {getTeamName(g.away_team, g.away_type)}
                      </div>
                      <div className="text-xs" style={{color:'#8a8279'}}>
                        {g.scheduled_date ? fmtDate(g.scheduled_date) : 'Date TBD'}
                        {g.home_type === 'world' || g.away_type === 'world' ? ' · vs World Team (AI)' : ''}
                      </div>
                    </div>
                    {g.status === 'final' && (
                      <div className="font-black text-sm" style={{color:'#1a1512'}}>
                        {g.home_score}-{g.away_score}
                      </div>
                    )}
                    <span className="text-xs font-bold px-2 py-0.5 rounded"
                          style={{background:ss.bg,color:ss.color}}>
                      {ss.label}{g.status==='pending' && isMyRequest ? ' (sent)' : ''}
                    </span>
                  </div>
                )
              })}
      ...
      <div>
        ...
        {allGames.filter(g => g.status !== 'declined').length === 0 ? (
          <div className="text-center py-8 rounded-xl" style={{background:'#faf8f5',border:'1px solid #d4cdc5'}}>
            <p className="text-sm" style={{color:'#8a8279'}}>No games scheduled yet.</p>
          </div>
        ) : (
          <div className="flex flex-col gap-1.5">
            {allGames
              ...
              .map(g => {
                const ss = STATUS_STYLE[g.status] || STATUS_STYLE.pending
                return (
                  <div key={g.id} className="flex items-center gap-3 px-4 py-2.5 rounded-lg"
                       style={{background:'#faf8f5',border:'1px solid #e2dcd5'}}>
                    <div className="w-24 text-xs font-semibold flex-shrink-0" style={{color:'#8a8279'}}>
                      {g.scheduled_date ? fmtDate(g.scheduled_date) : 'TBD'}
                    </div>
                    <div className="flex-1 text-sm font-semibold" style={{color:'#1a1512'}}>
                      {getTeamName(g.home_team, g.home_type)} vs {getTeamName(g.away_team, g.away_type)}
                    </div>
                    {g.status === 'final' && (
                      <div className="font-black text-sm" style={{color:'#1a1512'}}>
      ...
      {showModal && myTeamId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-lg rounded-2xl p-6"
               style={{background:'#faf8f5',border:'1px solid #d4cdc5',maxHeight:'90vh',overflowY:'auto'}}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-xl font-black" style={{color:'#1a1512'}}>Schedule Pre-Season Game</h2>
              <button onClick={() => { setShowModal(false); setSelectedDate(null); setSelectedOpponent(''); setMsg('') }}
                className="text-xl" style={{color:'#8a8279'}}>✕</button>
            </div>

            {/* Step 1: Pick date */}
            <div className="mb-5">
              <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{color:'#5c554e'}}>
                1. Choose Date (Oct 2–17)
              </div>
                ...
                {allDates.map(d => {
                  ...
                  return (
                    <button key={isoDate(d)}
                      disabled={!available}
                      onClick={() => { setSelectedDate(d); setSelectedOpponent('') }}
                      className="py-2 rounded-lg text-xs font-bold transition-all"
                      style={{
                        background: selected ? '#1a1512' : available ? '#e8e2d6' : '#f0ece5',
                        color: selected ? '#f5f1eb' : available ? '#1a1512' : '#c4bdb5',
                        border: selected ? '1px solid #1a1512' : '1px solid #d4cdc5',
            ...
            {selectedDate && (
              <div className="mb-5">
                ...
                <div className="flex gap-2">
                  {[{v:true,l:'Home (you host)'},{v:false,l:'Away (you travel)'}].map(o => (
                    <button key={String(o.v)} onClick={() => setIsHome(o.v)}
                      className="flex-1 py-2 rounded-lg text-xs font-bold"
                      style={{
                        background: isHome === o.v ? '#1a1512' : '#e8e2d6',
                        color: isHome === o.v ? '#f5f1eb' : '#1a1512',
            ...
            {selectedDate && (
              <div className="mb-4">
                ...
                <div className="flex gap-2 mb-3">
                  {[{v:'nba' as const,l:'NBA Team'},{v:'world' as const,l:'World Team (AI)'}].map(o => (
                    <button key={o.v} onClick={() => { setOpponentType(o.v); setSelectedOpponent('') }}
                      className="flex-1 py-2 rounded-lg text-xs font-bold"
                      style={{
                        background: opponentType === o.v ? '#1a1512' : '#e8e2d6',
                        color: opponentType === o.v ? '#f5f1eb' : '#1a1512',
                        border: '1px solid #d4cdc5',
                      }}>
                      {o.l}
                    </button>
                ...
                {(() => {
                  ...
                  if (!opponents.length) return (
                    <div className="text-xs py-2 text-center" style={{color:'#dc2626'}}>
                      No available opponents for this date
                    </div>
                  )
                  return (
                    <div className="grid grid-cols-2 gap-1.5 max-h-48 overflow-y-auto">
                      {opponents.map(t => (
                        <button key={t.id}
                          onClick={() => setSelectedOpponent(t.id)}
                          className="px-3 py-2 rounded-lg text-xs font-semibold text-left transition-all"
                          style={{
                            background: selectedOpponent === t.id ? '#1a1512' : '#e8e2d6',
                            color: selectedOpponent === t.id ? '#f5f1eb' : '#1a1512',
                            border: '1px solid #d4cdc5',
                          }}>
                          {t.name}
                          {opponentType === 'world' && (
                            <span className="block text-xs opacity-60">{t.country}</span>
                          )}
                        </button>
                      ))}
                    </div>
            ...
            {selectedOpponent && (
              <div className="mb-5">
                ...
                <input type="text" value={notes} onChange={e => setNotes(e.target.value)}
                  placeholder="Optional message to the opponent..."
            ...
            <button
              ...
              disabled={!selectedDate || !selectedOpponent || saving}
              className="w-full py-3 rounded-xl font-bold text-sm disabled:opacity-40"
              style={{background:'#1a1512',color:'#f5f1eb'}}>
              {saving ? 'Sending...' : opponentType === 'world' ? 'Schedule Game' : 'Send Request'}
            </button>
          </div>
        </div>
      )}
