...
const MSG_TYPE_STYLE: Record<string,{color:string,icon:string,label:string}> = {
  message:        {color:'#60a0ff',icon:'💬',label:'Message'},
  trade_proposal: {color:'#ffd040',icon:'🔄',label:'Trade Proposal'},
  staff_offer:    {color:'#40e080',icon:'👔',label:'Staff Offer'},
  system:         {color:'#8a7a6a',icon:'🔔',label:'System'},
}
...
export default function InboxPage() {
  ...
  if (authLoading) return (
    <div className="flex items-center justify-center h-64" style={{color:'#8a7a6a'}}>Loading...</div>
  )
  if (!user) return null
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6">
      <h1 className="text-xl font-bold mb-4" style={{color:'#f0ebe0'}}>✉️ Messages</h1>
      <div className="flex gap-4 h-[600px]">
        {/* Sidebar — GMs list */}
        <div className="w-64 flex-shrink-0 rounded-xl overflow-hidden flex flex-col"
             style={{border:'1px solid #3a3228',background:'#241f18'}}>
          <div className="px-4 py-3" style={{borderBottom:'1px solid #3a3228',background:'#120f0a'}}>
            <p className="text-xs font-semibold" style={{color:'#8a7a6a'}}>All GMs</p>
          </div>
          <div className="flex-1 overflow-y-auto">
            {profiles.map(p => {
              const tc = p.teams?.color ? '#'+p.teams.color : '#8a7a6a'
              const isOnline = onlineUsers.has(p.id)
              const isSelected = thread?.id === p.id
              return (
                <button key={p.id} onClick={() => setThread(p)}
                  className="w-full flex items-center gap-3 px-4 py-3 text-left transition-all"
                  style={{background:isSelected?'#3a3228':'transparent',
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    <div className="absolute bottom-0 right-0 w-2.5 h-2.5 rounded-full border-2"
                         style={{background:isOnline?'#40e080':'#5a4a3a',borderColor:'#241f18'}}></div>
                  </div>
                  <div className="min-w-0">
                    <div className="text-sm font-semibold truncate" style={{color:'#f0ebe0'}}>
        ...
        <div className="flex-1 rounded-xl flex flex-col overflow-hidden"
          ...
          {thread ? (
                ...
                <div className="w-8 h-8 rounded-full overflow-hidden"
                  ...
                  {thread.teams?.logo_url
                    ...
                    :<div className="w-full h-full flex items-center justify-center text-xs font-black"
                          style={{color:'#8a7a6a'}}>{thread.teams?.id?.slice(0,2)}</div>}
                </div>
                <div>
                  <div className="font-semibold text-sm" style={{color:'#f0ebe0'}}>
                    {thread.display_name||thread.teams?.name}
                  </div>
                  <div className="text-xs" style={{color:onlineUsers.has(thread.id)?'#40e080':'#5a4a3a'}}>
              ...
              <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
                {messages.length === 0 && (
                  <div className="flex-1 flex items-center justify-center">
                    <p className="text-sm" style={{color:'#5a4a3a'}}>No messages yet. Say hello!</p>
                  </div>
                )}
                {messages.map((m:any) => {
                    ...
                    <div key={m.id} className={`flex ${isMe?'justify-end':'justify-start'}`}>
                      <div className="max-w-[70%]">
                        ...
                        <div className="px-4 py-2.5 rounded-2xl text-sm"
                             style={{background:isMe?'#1e3a5f':'#3a3228',
                                     color:'#f0ebe0',
                        ...
                        <div className="text-xs mt-0.5 px-1"
                             style={{color:'#4a3a2a',textAlign:isMe?'right':'left'}}>
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-3">
              ...
              <p className="text-sm" style={{color:'#6a5a4a'}}>Select a GM to start chatting</p>
            </div>
          )}
        </div>
      </div>
