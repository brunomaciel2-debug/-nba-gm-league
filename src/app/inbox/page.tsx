...
export default function InboxPage() {
  ...
  const approveApp = async (msg: any) => {
    ...
    try {
      ...
      setActionMsg(`✅ ${msg.metadata.full_name} aprovado!`)
  ...
  const rejectApp = async (msg: any) => {
    ...
    try {
      ...
      const result = await res.json()
      if (!res.ok) throw new Error(result.error || 'Unknown error')
      setActionMsg(`❌ ${msg.metadata.full_name} rejeitado.`)
      await deleteMsg(msg.id)
    } catch (e: any) { setActionMsg(`❌ Erro: ${e.message}`) }
    setProcessing(null)
  }
    ...
    <div className="max-w-3xl mx-auto px-4 py-6">
          ...
          return (
            <button key={f} onClick={() => setFilter(f)}
              style={{
                ...
                border:`1px solid ${filter===f?color:'#d4cdc5'}`,
                background: filter===f ? color : '#f0ece5',
                color: filter===f ? '#fff' : '#5c554e',
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-12 rounded-2xl" style={{background:'#faf8f5',border:'1px solid #d4cdc5'}}>
          ...
          <p className="text-sm" style={{color:'#8a8279'}}>
            {filter === 'unread' ? 'No unread messages' : 'No messages in this category'}
          </p>
        </div>
      ) : (
              ...
              <div key={msg.id}>
                ...
                <div
                  ...
                  style={{
                    ...
                    borderLeft: `3px solid ${msg.read ? 'transparent' : accentColor}`,
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <span className="text-xs" style={{color:msg.read?'#b0a89e':accentColor,fontWeight:msg.read?400:700}}>
                    ...
                    {isCommissioner && msg.type === 'application' && msg.metadata?.application_id && (
                      <div className="px-6 py-3 flex items-center gap-3 flex-wrap"
                        ...
                        <span className="text-xs font-semibold" style={{color:'#5c554e'}}>
                          {msg.metadata.full_name} · {msg.metadata.email}
                        </span>
