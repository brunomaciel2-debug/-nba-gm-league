...
import type { Player, PlayerStats, Team, Game } from '@/lib/types'
...
export default async function TeamPage({ params }: { params: { id: string } }) {
  ...
  const [{ data: team }, { data: players }, { data: games }] = await Promise.all([
    ...
    supabase.from('players').select('*, player_stats(*)').eq('team_id', teamId).eq('status','active').order('usage', { ascending: false }),
    supabase.from('games').select('*, home:teams!games_home_team_fkey(id,name,color), away:teams!games_away_team_fkey(id,name,color)')
      .or(`home_team.eq.${teamId},away_team.eq.${teamId}`)
      .order('week_number', { ascending: false }).limit(10),
  ])
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="rounded-2xl p-6 mb-6"
           style={{ background:'#0f1e33', borderTop:'4px solid #'+t.color, border:'1px solid #1e3a5f' }}>
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex gap-6">
            <div className="text-center">
              <div className="text-3xl font-bold" style={{ color:'#40e080' }}>{t.wins}</div>
              <div className="text-xs" style={{ color:'#506070' }}>Wins</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold" style={{ color:'#e04040' }}>{t.losses}</div>
              <div className="text-xs" style={{ color:'#506070' }}>Losses</div>
            </div>
            <div className="text-center">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          {/* ROSTER */}
          <h2 className="text-sm font-semibold uppercase tracking-widest mb-3" style={{ color:'#506070' }}>Roster</h2>
              ...
              <thead>
                <tr style={{ background:'#060c18', borderBottom:'1px solid #1e3a5f' }}>
                  {['Player','Pos','PPG','RPG','APG','FG%','Salary'].map(h=>(
                    <th key={h} className="px-3 py-2 text-left font-semibold" style={{ color:'#7090b0' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {(players||[]).map((p:any, i:number) => {
                  const s: PlayerStats = p.player_stats?.[0] || {}
                  const gp = (s as any).games || 0
                  const ppg = gp>0?((s as any).pts/gp).toFixed(1):'—'
                  const rpg = gp>0?((s as any).reb/gp).toFixed(1):'—'
                  const apg = gp>0?((s as any).ast/gp).toFixed(1):'—'
                  const fgpct = (s as any).fga>0?(((s as any).fgm/(s as any).fga)*100).toFixed(1)+'%':'—'
                  return (
                    <tr key={p.id} style={{ background:i%2===0?'#0f1e33':'#0c1a2c', borderBottom:'1px solid #0a1628' }}>
                      <td className="px-3 py-2">
                        <Link href={`/player/${p.id}`} className="font-semibold text-white no-underline hover:text-blue-400">
                          {p.name}
                        </Link>
                      </td>
                      <td className="px-3 py-2" style={{ color:'#7090b0' }}>{p.pos}</td>
                      <td className="px-3 py-2 font-bold" style={{ color:'#ffa040' }}>{ppg}</td>
                      <td className="px-3 py-2" style={{ color:'#40e080' }}>{rpg}</td>
                      <td className="px-3 py-2" style={{ color:'#60a0ff' }}>{apg}</td>
                      <td className="px-3 py-2" style={{ color:'#c0ccd8' }}>{fgpct}</td>
          ...
          {/* RECENT GAMES */}
          <h2 className="text-sm font-semibold uppercase tracking-widest mb-3" style={{ color:'#506070' }}>Recent Games</h2>
          <div className="flex flex-col gap-2">
            {(games||[]).filter((g:any)=>g.status==='final').map((g:any)=>{
              ...
              return (
                <Link key={g.id} href={`/game/${g.id}`} className="no-underline">
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
                    ...
                    <span className="text-xs" style={{ color:'#7090b0' }}>{isHome?'vs':'@'} {opp?.name||'—'}</span>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="rounded-xl p-4" style={{ background:'#0f1e33',border:'1px solid #1e3a5f' }}>
            ...
            <div className="mb-3">
              ...
              <div className="h-2 rounded-full overflow-hidden" style={{ background:'#1e3a5f' }}>
                <div className="h-full rounded-full" style={{ width:Math.min(100,used/cap*100)+'%', background: space>0?'#3a8adf':'#e04040' }}></div>
              </div>
          ...
          <div className="rounded-xl p-4" style={{ background:'#0a2a10',border:'1px solid #1a5a2a' }}>
            ...
            <p className="text-xs mb-3" style={{ color:'#7090b0' }}>Submit your depth chart and tactical orders for the upcoming week.</p>
