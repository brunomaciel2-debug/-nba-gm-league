    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="grid md:grid-cols-3 gap-6">
        <div className="md:col-span-2">
          {/* ROSTER TABLE — client component with sort/filter */}
          <RosterTable players={players || []} teamColor={color} />

          {/* RECENT GAMES */}
          <h2 className="text-xs font-semibold uppercase tracking-widest mb-3 mt-6" style={{ color: '#506070' }}>Recent Results</h2>
              ...
              return (
                <Link key={g.id} href={`/game/${g.id}`} className="no-underline">
                  <div className="flex items-center gap-3 px-4 py-3 rounded-xl"
                    ...
                    <span className="text-xs flex-1" style={{ color: '#7090b0' }}>{isHome ? 'vs' : '@'} {opp?.name}</span>
                    <span className="text-sm font-bold" style={{ color: win ? '#40e080' : '#e04040' }}>{us}–{them}</span>
                    <span className="text-xs px-2 py-1 rounded" style={{ background: '#1e3a5f', color: '#60a0ff' }}>Box →</span>
                  </div>
                </Link>
              )
            })}
        ...
        <div className="flex flex-col gap-4">
          <div className="rounded-xl p-4" style={{ background: '#0f1e33', border: '1px solid #1e3a5f' }}>
            ...
            <div className="flex justify-between text-xs">
              ...
              <span style={{ color: space > 0 ? '#40e080' : '#e04040' }}>{space > 0 ? '+' : ''}{capFmt(space)}</span>
            </div>
          </div>
