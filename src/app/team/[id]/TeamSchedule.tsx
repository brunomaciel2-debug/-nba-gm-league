...
function isPreseason(d: Date) { return d >= PRESEASON_START && d <= PRESEASON_END }
...
export default function TeamSchedule({
  ...
  const isPreseasonPeriod = today >= PRESEASON_START && today <= PRESEASON_END
  ...
  const filters: {key:Filter, label:string}[] = [
    ...
    {key:'played',label:'Played'},{key:'home',label:'Home'},{key:'away',label:'Away'},
  ]
  ...
  for (const g of filtered) {
    ...
    const key = d ? d.toLocaleDateString('en-US',{month:'long',year:'numeric'}) : 'TBD'
  ...
  const fmtTime = (iso:string) => new Date(iso).toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit',timeZone:'Europe/Lisbon'})

  // Preseason scheduling logic
  ...
  const getAvailableOpps = (date: Date) => {
    ...
    return {
      nba: nbaTeams.filter(t=>t.id!==teamId&&!busy.has(t.id)),
      world: worldTeams.filter(t=>!busy.has(t.id)),
    }
  }
  ...
  const handleSchedule = async () => {
    ...
    // Send inbox notification if NBA game
    if (oppType==='nba') {
      await supabase.from('inbox_messages').insert({
        to_team_id: selectedOpp,
        from_team_id: myTeamId,
        subject: 'Pre-Season Game Request',
        body: `${teams[myTeamId]?.name||myTeamId} has challenged you to a pre-season game on ${fmtDate(isoDate(selectedDate)+'T12:00:00')}. ${note?`Message: "${note}"`:''}`,
        type: 'preseason_request',
        metadata: { date: isoDate(selectedDate), requesting_team: myTeamId },
      })
    }
    setMsg(oppType==='world'?'✅ Game scheduled!':'✅ Request sent! Opponent will be notified.')
    // Refresh
  ...
  const TYPE_BADGE: Record<string,{label:string,bg:string,color:string}> = {
    ...
    regular:   {label:'Regular',   bg:'#f0fdf4',color:'#15803d'},
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        {myTeamId && isPreseasonPeriod && myActivePS.length >= MAX_PRESEASON && (
          <span className="text-xs font-semibold px-3 py-1.5 rounded-lg"
            ...
            ⚠️ Max 5 pre-season games reached
          </span>
        )}
      </div>

      {/* Pre-season games pending */}
      {isPreseasonPeriod && preseasonGames.filter(g=>g.status==='pending'&&g.requested_by!==teamId).length > 0 && (
        <div className="mb-4 p-3 rounded-xl" style={{background:'#fef9c3',border:'1px solid #b45309'}}>
          <div className="text-xs font-bold mb-2" style={{color:'#b45309'}}>⏳ Pending requests to accept</div>
          {preseasonGames.filter(g=>g.status==='pending'&&g.requested_by!==teamId).map((g:any) => (
            <div key={g.id} className="flex items-center justify-between gap-3 py-1.5">
              ...
              <div className="flex gap-2">
                <button onClick={async()=>{ await supabase.from('preseason_games').update({status:'scheduled'}).eq('id',g.id); const {data:pg}=await supabase.from('preseason_games').select('*').eq('season','2025-26').order('scheduled_date'); setAllPreseasonGames(pg||[]); setPreseasonGames((pg||[]).filter((x:any)=>x.home_team===teamId||x.away_team===teamId)) }}
                  className="px-2 py-1 rounded text-xs font-bold" style={{background:'#15803d',color:'#fff'}}>Accept</button>
                <button onClick={async()=>{ await supabase.from('preseason_games').update({status:'declined'}).eq('id',g.id); const {data:pg}=await supabase.from('preseason_games').select('*').eq('season','2025-26').order('scheduled_date'); setAllPreseasonGames(pg||[]); setPreseasonGames((pg||[]).filter((x:any)=>x.home_team===teamId||x.away_team===teamId)) }}
                  className="px-2 py-1 rounded text-xs font-bold" style={{background:'#dc2626',color:'#fff'}}>Decline</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {sortedMonths.map(month=>(
        <div key={month} className="mb-6">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-xs" style={{color:'#8a8279'}}>{byMonth[month].length} game{byMonth[month].length!==1?'s':''}</span>
          </div>
                ...
                return (
                  <div key={g.id} className="flex items-center gap-3 px-3 py-2.5 rounded-lg"
                    ...
                    <div className="w-24 flex-shrink-0">
                      ...
                      {g.week_number>0&&<div className="text-xs" style={{color:'#8a8279'}}>Wk {g.week_number}</div>}
                    ...
                    <div className="flex-shrink-0 text-right">
                      {isPlayed?(
                        ...
                        <span className="text-xs px-2 py-0.5 rounded" style={{background:'#f0ece5',color:'#8a8279'}}>Scheduled</span>
                      )}
                    </div>
                  </div>
                )
      ...
      {/* MODAL: Schedule pre-season game */}
      {showModal&&myTeamId&&(
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{background:'rgba(0,0,0,0.65)'}}>
          <div className="w-full max-w-md rounded-2xl p-6" style={{background:'#faf8f5',border:'1px solid #d4cdc5',maxHeight:'90vh',overflowY:'auto'}}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="text-lg font-black" style={{color:'#1a1512'}}>🏀 Schedule Pre-Season Game</h2>
              <button onClick={()=>{setShowModal(false);setSelectedDate(null);setSelectedOpp('');setMsg('')}} style={{color:'#8a8279',fontSize:20}}>✕</button>
            </div>

            {/* Step 1: Calendar */}
            <div className="mb-5">
              <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{color:'#5c554e'}}>1. Choose Date (Oct 2–17)</div>
              ...
              <div className="flex gap-3 mt-2 text-xs">
                <span className="flex items-center gap-1"><span style={{display:'inline-block',width:10,height:10,borderRadius:2,background:'#e8e2d6'}}></span>Available</span>
                <span className="flex items-center gap-1"><span style={{display:'inline-block',width:10,height:10,borderRadius:2,background:'#fee2e2'}}></span>Unavailable</span>
              </div>
            </div>
            ...
            {selectedDate&&(
              <div className="mb-5">
                <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{color:'#5c554e'}}>2. Your Role</div>
                <div className="flex gap-2">
                  {[{v:true,l:'🏠 Home'},{v:false,l:'✈️ Away'}].map(o=>(
            ...
            {/* Step 3: Opponent type */}
            {selectedDate&&(
              <div className="mb-4">
                <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{color:'#5c554e'}}>3. Opponent</div>
                ...
                {(()=>{
                  ...
                  if(!opps.length) return <div className="text-xs text-center py-2" style={{color:'#dc2626'}}>No available opponents for this date</div>
                  return (
            ...
            {selectedOpp&&(
              <div className="mb-4">
                <div className="text-xs font-bold uppercase tracking-widest mb-2" style={{color:'#5c554e'}}>4. Message (optional)</div>
            ...
            <button onClick={handleSchedule} disabled={!selectedDate||!selectedOpp||saving}
              ...
              {saving?'Sending...':(oppType==='world'?'Schedule Game':'Send Request →')}
            </button>
          </div>
        </div>
      )}
