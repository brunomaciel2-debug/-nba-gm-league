...
const GYM_CONFIG: Record<GymGrade,{label:string,color:string,bg:string,speed:number,recovery:number,risk:number,fa:number,cost:number,unlocks:string[],desc:string}> = {
  F:{label:'Grade F',color:'#dc2626',bg:'#fee2e2',speed:5, recovery:-5, risk:0,  fa:0,  cost:50000,   unlocks:[],            desc:'Temporary rented facility'},
  E:{label:'Grade E',color:'#b45309',bg:'#fef3c7',speed:7, recovery:0,  risk:0,  fa:0,  cost:150000,  unlocks:[],            desc:'Entry-level NBA facility'},
  D:{label:'Grade D',color:'#ca8a04',bg:'#fefce8',speed:9, recovery:3,  risk:0,  fa:0,  cost:300000,  unlocks:['Playmaking'],desc:'2 courts, video room'},
  C:{label:'Grade C',color:'#15803d',bg:'#dcfce7',speed:12,recovery:7,  risk:-5, fa:0,  cost:600000,  unlocks:['Shooting'],  desc:'3 courts, physiotherapy'},
  B:{label:'Grade B',color:'#1d4ed8',bg:'#dbeafe',speed:15,recovery:13, risk:-10,fa:5,  cost:1200000, unlocks:['Mental'],    desc:'Hydrotherapy, cryotherapy'},
  A:{label:'Grade A',color:'#6d28d9',bg:'#ede9fe',speed:19,recovery:20, risk:-18,fa:12, cost:2500000, unlocks:['Analytics'], desc:'World class campus'},
}
...
export default function FacilitiesTab({teamId,teamColor,arenaName,arenaCapacity,cash=45000000}:{
  ...
  const handleUpgrade=async()=>{
    ...
    setMsg(`Upgrade to ${upg.next} started — ready in ${upg.weeks} weeks.`)
    setUpgrading(false)
  }
    ...
    <div>
      ...
      {view==='gym' && (
        <div style={{display:'flex',gap:16,alignItems:'flex-start'}}>
          ...
          {/* LEFT: grade progress + extras */}
          <div style={{flex:1,minWidth:0}}>

            {msg && (
              <div style={{marginBottom:12,padding:'8px 12px',borderRadius:8,background:'#dcfce7',color:'#15803d',fontSize:12,fontWeight:600,border:'1px solid #bbf7d0'}}>
                ✓ {msg}
              </div>
            )}
            ...
            <div style={{background:'#faf8f5',border:'1px solid #d4cdc5',borderRadius:12,padding:16,marginBottom:12}}>
              <div style={{fontSize:11,fontWeight:700,textTransform:'uppercase',letterSpacing:'1px',color:'#8a8279',marginBottom:12}}>
                Practice Facility Grade
              </div>
              <div style={{display:'flex',gap:6,alignItems:'stretch',marginBottom:12}}>
                {GRADES.map((g,i)=>{
                  const gc=GYM_CONFIG[g]
                  const isCurrent=g===facility.gym_grade
                  const isPast=i<gradeIdx
                  const isFuture=i>gradeIdx
                  return (
                    <div key={g} style={{flex:1,borderRadius:8,padding:'8px 4px',textAlign:'center',
                      background:isCurrent?gc.bg:isPast?gc.color+'22':'#f0ece5',
                      border:`1px solid ${isCurrent?gc.color:isPast?gc.color+'44':'#e2dcd5'}`,
                      opacity:isFuture?0.5:1}}>
                      <div style={{fontSize:14,fontWeight:900,color:isCurrent?gc.color:isPast?gc.color:'#b0a89e'}}>{g}</div>
                      {isCurrent && <div style={{fontSize:8,color:gc.color,fontWeight:700,marginTop:1}}>CURRENT</div>}
                    </div>
                  )
                })}
              </div>
              ...
              <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10}}>
                ...
                <div>
                  <div style={{fontSize:15,fontWeight:700,color:facility.gym_under_construction?'#1d4ed8':cfg.color}}>
                    {facility.gym_under_construction?`Upgrading to ${upg?.next}...`:cfg.label}
                  </div>
                  <div style={{fontSize:12,color:'#5c554e'}}>{cfg.desc}</div>
                </div>
              </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:6}}>
                {[
                  {label:'Slot speed',val:`+${cfg.speed}%/wk`,color:cfg.color},
              ...
              {facility.gym_under_construction && (
                <div style={{marginTop:10,padding:'8px 10px',background:'#dbeafe',borderRadius:8,border:'1px solid #93c5fd',fontSize:11,color:'#1d4ed8'}}>
                  🚧 Under construction · Ready: {facility.gym_upgrade_ends_at?new Date(facility.gym_upgrade_ends_at).toLocaleDateString('en-US',{month:'short',day:'numeric',year:'numeric'}):'TBD'}
                </div>
              )}
            </div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {EXTRAS.map(ef=>{
                ...
                return (
                  <div key={ef.key} style={{
                    background:built?'#f0fdf4':'#faf8f5',
                    border:`1px solid ${built?'#bbf7d0':'#d4cdc5'}`,
                    borderLeft:`3px solid ${built?'#15803d':'#d4cdc5'}`,
                    ...
                    {!built && isGM && (
                      <button onClick={()=>handleBuild(ef.key,ef.cost,ef.monthly)}
                        ...
                        style={{padding:'4px 10px',fontSize:11,fontWeight:600,borderRadius:6,border:'none',flexShrink:0,
                          background:cash>=ef.cost?teamColor:'#e2dcd5',
                          color:cash>=ef.cost?'#fff':'#8a8279',
                          cursor:cash>=ef.cost?'pointer':'not-allowed'}}>
                        Build
                      </button>
                    )}
                  </div>
          ...
          <div style={{width:200,flexShrink:0}}>
            {upg && nextCfg && !facility.gym_under_construction ? (
              <div style={{background:'#faf8f5',border:'1px solid #d4cdc5',borderRadius:12,padding:14}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:10}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:10,color:'#8a8279'}}>{nextCfg.desc}</div>
                  </div>
                </div>
                {[
                ...
                {isGM && (
                  <button onClick={handleUpgrade} disabled={!upg||cash<upg.cost||upgrading}
                    style={{width:'100%',marginTop:10,padding:'8px',fontSize:12,fontWeight:700,borderRadius:8,border:'none',
                      background:cash>=upg.cost?cfg.color:'#e2dcd5',
                      color:cash>=upg.cost?'#fff':'#8a8279',
                      cursor:cash>=upg.cost?'pointer':'not-allowed'}}>
                    {upgrading?'...':`Upgrade — ${fmtM(upg.cost)}`}
                  </button>
                )}
                {cash<upg.cost && <div style={{fontSize:10,color:'#dc2626',marginTop:4,textAlign:'center'}}>Insufficient funds</div>}
              </div>
              ...
              <div style={{background:'#ede9fe',border:'1px solid #6d28d9',borderRadius:12,padding:14,textAlign:'center'}}>
                ...
                <div style={{fontSize:13,fontWeight:700,color:'#6d28d9'}}>World Class</div>
                <div style={{fontSize:11,color:'#5c554e',marginTop:4}}>Maximum grade achieved</div>
              </div>
            ) : facility.gym_under_construction ? (
              <div style={{background:'#dbeafe',border:'1px solid #93c5fd',borderRadius:12,padding:14,textAlign:'center'}}>
                <div style={{fontSize:20,marginBottom:6}}>🚧</div>
                <div style={{fontSize:13,fontWeight:700,color:'#1d4ed8'}}>In Progress</div>
                <div style={{fontSize:11,color:'#5c554e',marginTop:4}}>
                  {facility.gym_upgrade_ends_at?new Date(facility.gym_upgrade_ends_at).toLocaleDateString('en-US',{month:'short',day:'numeric'}):'TBD'}
                </div>
              </div>
            ) : null}
          </div>
