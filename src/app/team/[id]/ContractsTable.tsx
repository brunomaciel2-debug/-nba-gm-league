...
export default function ContractsTable({ teamId, teamColor }: { teamId: string, teamColor: string }) {
  ...
  if (loading) return <div className="p-8 text-center text-sm" style={{ color: '#6a5a4a' }}>Loading contracts...</div>
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: 'Luxury Tax', value: currentTotal > LUX_TAX ? '+' + fmt(currentTotal - LUX_TAX) : fmt(LUX_TAX - currentTotal) + ' away', sub: currentTotal > LUX_TAX ? 'Paying luxury tax' : 'Below tax line', color: currentTotal > LUX_TAX ? '#e04040' : '#8a7a6a' },
        ].map(card => (
          <div key={card.label} className="rounded-xl p-4" style={{ background: '#241f18', border: '1px solid #3a3228' }}>
            <div className="text-xs mb-1" style={{ color: '#6a5a4a' }}>{card.label}</div>
            <div className="text-xl font-black" style={{ color: card.color }}>{card.value}</div>
            <div className="text-xs mt-0.5" style={{ color: '#5a4a3a' }}>{card.sub}</div>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl p-4 mb-6" style={{ background: '#241f18', border: '1px solid #3a3228' }}>
        <div className="flex justify-between text-xs mb-2" style={{ color: '#6a5a4a' }}>
          <span>2025-26 Payroll</span>
          <span style={{ color: barColor(capPct) }}>{fmt(currentTotal)} / {fmt(SALARY_CAP)} cap</span>
        </div>
        <div className="h-3 rounded-full overflow-hidden" style={{ background: '#3a3228' }}>
          <div className="h-full rounded-full transition-all"
               style={{ width: Math.min(capPct, 130) + '%', background: barColor(capPct), maxWidth: '100%' }} />
        </div>
        <div className="flex justify-between text-xs mt-1" style={{ color: '#4a3a2a' }}>
          <span>$0</span>
      ...
      <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #2a2218' }}>
        <div className="overflow-x-auto">
          <table className="w-full text-xs" style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#120f0a' }}>
                <th className="px-4 py-3 text-left sticky left-0" style={{ color: '#6a5a4a', background: '#120f0a', borderBottom: '1px solid #2a2218', minWidth: 160 }}>Player</th>
                <th className="px-3 py-3 text-center w-12" style={{ color: '#6a5a4a', background: '#120f0a', borderBottom: '1px solid #2a2218' }}>Pos</th>
                {SEASONS.map((s, i) => (
                  <th key={s} className="px-4 py-3 text-right whitespace-nowrap"
                      style={{ color: i === 0 ? '#F5A623' : '#6a5a4a', background: '#120f0a', borderBottom: '1px solid #2a2218', minWidth: 100 }}>
                    {s}{i === 0 ? ' ★' : ''}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {rows.map((p, i) => (
                <tr key={p.id} style={{ background: i % 2 === 0 ? '#1a1610' : '#181410', borderBottom: '1px solid #242018' }}>
                  <td className="px-4 py-2.5 sticky left-0" style={{ background: i % 2 === 0 ? '#1a1610' : '#181410' }}>
                    <span className="font-semibold" style={{ color: '#f0ebe0' }}>{p.name}</span>
                  </td>
                  <td className="px-3 py-2.5 text-center" style={{ color: '#6a5a4a' }}>{p.pos}</td>
                  {SEASONS.map(s => {
                    ...
                    return (
                      <td key={s} className="px-4 py-2.5 text-right font-semibold"
                          style={{ color: sal ? (isExpiring && SEASONS.indexOf(s) < 4 ? '#ffd040' : '#c0b8a8') : '#3a3228' }}>
                        {sal ? fmt(sal) : '—'}
              ...
              {/* Cap line */}
              <tr style={{ background: '#1a1610', borderTop: '1px solid #2a2218' }}>
                <td className="px-4 py-2 sticky left-0" style={{ color: '#4a3a2a', background: '#1a1610', fontSize: 11 }}>Salary Cap</td>
                <td style={{ background: '#1a1610' }}></td>
                {SEASONS.map(s => (
                  <td key={s} className="px-4 py-2 text-right" style={{ color: '#4a3a2a', fontSize: 11 }}>${(SALARY_CAP / 1000000).toFixed(0)}M</td>
                ))}
              </tr>

              {/* Space/over row */}
              <tr style={{ background: '#1a1610' }}>
                <td className="px-4 py-2 sticky left-0" style={{ color: '#4a3a2a', background: '#1a1610', fontSize: 11 }}>Space / Over</td>
                <td style={{ background: '#1a1610' }}></td>
                {SEASONS.map(s => {
      ...
      <p className="text-xs mt-3" style={{ color: '#4a3a2a' }}>
