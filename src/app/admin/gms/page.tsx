...
export default function ManageGMsPage() {
  ...
  const fireGM = async (gm: any) => {
    const confirmMsg = isPT
      ? `Despedir ${gm.display_name} dos ${gm.teams?.name}?`
      : `Fire ${gm.display_name} from ${gm.teams?.name}?`
    if (!confirm(confirmMsg)) return
    setProcessing(gm.id); setMsg('')
    try {
      const res = await fetch('/api/admin/fire-gm',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({user_id:gm.id,team_id:gm.team_id})})
      const result = await res.json()
      if (!res.ok) throw new Error(result.error||'Unknown error')
      setMsg(isPT ? `✅ ${gm.display_name} despedido com sucesso.` : `✅ ${gm.display_name} fired successfully.`)
      await load()
    } catch(e:any) {
      setMsg(`❌ ${isPT?'Erro':'Error'}: ${e.message}`)
    }
    setProcessing(null)
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold" style={{color:'#1a1512'}}>
            👥 {isPT ? 'Gerir GMs' : 'Manage GMs'}
          </h1>
          <p className="text-sm" style={{color:'#6b5f4e'}}>
            {gms.length} {isPT?'GMs activos':'active GMs'} · {vacancies.length} {isPT?'equipas vagas':'vacant teams'}
          </p>
        </div>
      ...
      <h2 className="text-xs font-semibold uppercase tracking-widest mb-3" style={{color:'#6b5f4e'}}>
        🔴 {isPT?'Equipas Vagas':'Vacant Teams'} ({vacancies.length})
      </h2>
