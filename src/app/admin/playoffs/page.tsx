...
export default function AdminPlayoffsPage() {
  ...
  const generate = async () => {
    if (!confirm('This will generate the playoff bracket based on current standings. Continue?')) return
    setLoading(true)
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <h1 className="text-xl font-bold mb-2" style={{color:'#1a1512'}}>🏆 Generate Playoff Bracket</h1>
      ...
      <div className="rounded-xl p-5 mb-6" style={{background:'#faf8f5',border:'1px solid #d4cdc5'}}>
        <div className="text-xs font-bold uppercase tracking-widest mb-3" style={{color:'#5c554e'}}>What gets created:</div>
      ...
      {result && (
        <div className="mt-4 rounded-xl p-4"
          ...
          {result.error
            ? <p className="text-sm font-semibold" style={{color:'#dc2626'}}>Error: {result.error}</p>
            : <div>
                <p className="text-sm font-bold" style={{color:'#15803d'}}>✓ Bracket generated successfully!</p>
                <p className="text-xs mt-1" style={{color:'#166534'}}>{result.created} series created.</p>
