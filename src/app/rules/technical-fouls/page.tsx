...
export default function TechnicalFoulsRulesPage() {
  ...
  const rules = isPT ? [
    ...
    { icon:'⚠️', title:'Foul Trouble — First Half Only', desc:'If a player picks up his 3rd personal foul still in the 1st quarter, or his 4th by the end of the 2nd quarter, the coach protects him: a portion of his remaining planned minutes shifts to whichever teammate has played the least that game. From the 3rd quarter onward it no longer counts as foul trouble — by then there\'s no benefit to saving minutes, just managing fouls normally. Shows a "FOUL TROUBLE" tag on the box score, and the PF column turns orange.' },
    ...
    <div style={{ maxWidth:800, margin:'0 auto', padding:'24px 16px' }}>
      ...
      <div style={{ marginTop:20, padding:'16px 18px', borderRadius:12, background:'#fee2e2', border:'1px solid #fca5a5' }}>
        <div style={{ fontSize:12, color:'#dc2626', lineHeight:1.7 }}>
          {isPT
            ? '2 técnicas no mesmo jogo = expulsão imediata · 3ª falta no 1º período ou 4ª até ao 2º período = foul trouble (minutos reduzidos, só na 1ª metade) · 6ª falta pessoal = desqualificação · Época regular: 16 técnicas = 1 jogo, +1 jogo a cada 2 adicionais · Playoffs: 7 técnicas, mesma lógica.'
            : '2 technicals in the same game = immediate ejection · 3rd foul in the 1st quarter or 4th by the 2nd quarter = foul trouble (reduced minutes, first half only) · 6th personal foul = disqualification · Regular season: 16 technicals = 1 game, +1 game every 2 more · Playoffs: 7 technicals, same logic.'}
        </div>
      </div>
    </div>
  )
