...
export default function OrdersRulesPage() {
  ...
  const sections = isPT ? [
    ...
    { title:'Intensidade de Treino', color:'#6d28d9', icon:'🏋️', rules:['Cinco níveis: Descanso (+150% regeneração de saúde), Leve (+25%), Normal (regeneração completa), Intenso (−50%), Carga Máxima (−75%, risco de lesão).','Intensidade mais alta aguça a forma mas reduz a recuperação entre jogos.'] },
  ] : [
    ...
    { title:'Training Intensity', color:'#6d28d9', icon:'🏋️', rules:['Five levels: Rest (+150% health regen), Light (+25%), Normal (full regen), Intense (−50%), Max Load (−75%, injury risk).','Higher intensity sharpens readiness but reduces recovery between games.'] },
  ]
    ...
    <div style={{ maxWidth:800, margin:'0 auto', padding:'24px 16px' }}>
      ...
      <div style={{ marginTop:20, padding:'16px 18px', borderRadius:12, background:'#dbeafe', border:'1px solid #93c5fd' }}>
        <div style={{ fontSize:12, color:'#1d4ed8', lineHeight:1.7 }}>
          {isPT ? 'Cada posição precisa de exatamente 48 minutos · Função com Bola ≠ Prioridade Ofensiva · Ritmo, Taxa 3P, Estilo Ofensivo & Defensivo definem o plano de jogo · Intensidade de treino troca recuperação por forma.' : 'Each position needs exactly 48 minutes · Ball Role ≠ Offensive Priority · Pace, 3PT Rate, Attack & Defense Style shape your game plan · Training Intensity trades recovery for readiness.'}
        </div>
      </div>
    </div>
  )
