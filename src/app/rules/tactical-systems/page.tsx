...
export default function TacticalSystemsRulesPage() {
  ...
  const rules = isPT ? [
    ...
    { icon:'🔓', title:'Desbloqueio de Baixo para Cima', desc:'As techs de nível 1 estão sempre disponíveis. Dentro de cada nível, a tech desbloqueia uma a uma pela ordem da árvore: a 1ª tech de qualquer nível precisa de 2 techs dominadas no nível abaixo; a 2ª precisa de 3; a 3ª precisa de 4; e assim por diante. Como cada nível tem menos techs que o anterior (5→4→3→2→1), chegar ao topo (nível 5, maestria total) exige dominar quase a árvore toda.' },
    ...
    { icon:'🔓', title:'Unlocking Bottom-Up', desc:"Level 1's 5 techs are always available. Within each level, techs unlock one by one in tree order: the 1st tech of any level needs 2 mastered techs from the level below; the 2nd needs 3; the 3rd needs 4; and so on. Since each level has fewer techs than the one below it (5→4→3→2→1), reaching the very top (level 5, full mastery) requires mastering nearly the whole tree." },
