...
export default async function GamePage({ params }: { params: { id: string } }) {
  ...
  if (!game) return <div className="p-8 text-center" style={{ color:'#7090b0' }}>Game not found.</div>
  ...
  const BoxTable = ({ rows, team }: { rows: any[], team: any }) => {
    ...
    return (
      <div className="mb-6">
        <div className="px-4 py-2 font-bold text-sm" style={{ background:'#060c18', borderBottom:'1px solid #1e3a5f', color:'#'+team.color }}>
          {team.name}
        </div>
            ...
            <thead>
              <tr style={{ background:'#060c18', borderBottom:'1px solid #1e3a5f' }}>
                {['Player','MIN','PTS','REB','AST','STL','BLK','FG','FG%','3P','3P%','FT','FT%','TO'].map(h=>(
                  <th key={h} className="px-2 py-2 text-right font-semibold first:text-left" style={{ color:'#7090b0' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {rows.map((r:any,i:number)=>(
                <tr key={r.id} style={{ background:i%2===0?'#0f1e33':'#0c1a2c', borderBottom:'1px solid #0a1628' }}>
                  <td className="px-2 py-2 font-medium text-white">{r.players?.name||'—'} <span style={{ color:'#506070' }}>{r.players?.pos}</span></td>
                  <td className="px-2 py-2 text-right" style={{ color:'#7090b0' }}>{r.mins}</td>
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center rounded-2xl overflow-hidden mb-6"
        ...
        {[{t:home,sc:game.home_score,side:'home'},{t:away,sc:game.away_score,side:'away'}].map(({t,sc,side},i)=>(
          <div key={side} className="flex-1 p-6 text-center" style={i===0?{borderRight:'1px solid #1e3a5f'}:{}}>
            <div className="text-xs font-bold mb-1" style={{ color:'#'+t.color }}>{t.id}</div>
            <div className="text-sm font-medium mb-2" style={{ color:'#c0ccd8' }}>{t.name}</div>
            <div className="text-5xl font-black"
                 style={{ color: winner===side?'#fff':'#7090b0' }}>{sc??'—'}</div>
          </div>
        ))}
      </div>
      ...
      {/* Tabs — box + PBP */}
      <div className="mb-6">
        <h2 className="text-sm font-semibold uppercase tracking-widest mb-4" style={{ color:'#506070' }}>Box Score</h2>
      ...
      {(pbp||[]).length > 0 && (
        <div>
          <h2 className="text-sm font-semibold uppercase tracking-widest mb-4" style={{ color:'#506070' }}>Play-by-Play</h2>
          <div className="rounded-xl overflow-hidden" style={{ border:'1px solid #1e3a5f' }}>
            {[1,2,3,4].map(q=>{
              ...
              return (
                <div key={q}>
                  <div className="px-4 py-2 text-xs font-bold uppercase tracking-widest"
                       style={{ background:'#060c18',borderBottom:'1px solid #1e3a5f',color:'#506070' }}>
                    Q{q}
                  </div>
                  {qPlays.map((p:any,i:number)=>(
                    <div key={p.id} className="flex gap-3 px-4 py-2 text-xs"
                      ...
                      <span className="min-w-[36px] font-mono" style={{ color:'#405060' }}>{p.time_left}</span>
                      ...
                      <span className="font-bold" style={{ color:'#506070' }}>{p.home_score}–{p.away_score}</span>
                    </div>
                  ))}
                </div>
              )
