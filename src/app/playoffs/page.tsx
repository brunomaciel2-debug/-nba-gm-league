...
function Seed({ team, seed }: { team: any, seed: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5 px-3 py-2.5 rounded"
      ...
      <span className="truncate" style={{fontSize:15,fontWeight:600,color: team?'#1a1512':'#9c9088',minWidth:0}}>
      ...
      <span className="flex-shrink-0" style={{fontSize:14,color:'#8a8279',marginLeft:'auto'}}>{team?`${team.wins}-${team.losses}`:''}</span>
    </div>
  )
}
