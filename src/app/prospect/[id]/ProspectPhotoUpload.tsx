...
export default function ProspectPhotoUpload({ prospectId, currentPhoto }: { prospectId: string, currentPhoto?: string }) {
  ...
  const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const path = `prospects/${prospectId}.${ext}`
      ...
      const { error: dbErr } = await supabase.from('prospects').update({ photo_url: publicUrl }).eq('id', prospectId)
