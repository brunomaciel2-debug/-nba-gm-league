...
const ATTR_TIPS: Record<string,string> = {
  ...
  usage:'Usage Rate — how often this player is involved in offensive plays.',
  ...
  speed:'Speed — straight-line running speed.',
  agility:'Agility — change-of-direction quickness.',
  strength:'Strength — physical power and ability to handle contact.',
  close_shot:'Close Shot — finishing ability in the paint outside the rim.',
  standing_dunk:'Standing Dunk — ability to dunk without a running start.',
  ...
  trash_talk:'Trash Talk — ability to get in opponents\' heads.',
}
...
const ATTR_GROUPS = [
  { label: 'Scoring',       color: '#b45309', attrs: [
    ...
    {key:'draw_foul',label:'Draw Foul'},{key:'usage',label:'Usage Rate'},
    {key:'close_shot',label:'Close Shot'},{key:'standing_dunk',label:'Standing Dunk'},
  ]},
  ...
  { label: 'Athleticism',   color: '#6d28d9', attrs: [
    ...
    {key:'speed',label:'Speed'},{key:'agility',label:'Agility'},{key:'strength',label:'Strength'},
  ]},
  ...
  { label: 'Psychological', color: '#b45309', attrs: [
    ...
    {key:'crowd_effect',label:'Crowd Effect'},{key:'streaky',label:'Streaky'},{key:'trash_talk',label:'Trash Talk'},
  ]},
]
  ...
  if (!revealed) {
    return (
      <div className="flex items-center gap-2">
        <div className="flex-1 h-2 rounded-full overflow-hidden" style={{background:'#e2dcd5'}}>
          <div style={{
            width:'30%',height:'100%',borderRadius:9999,
            background:'repeating-linear-gradient(45deg,#d4cdc5,#d4cdc5 4px,#e2dcd5 4px,#e2dcd5 8px)',
          }}/>
        </div>
        <span className="text-xs font-bold w-7 text-right" style={{color:'#c8c0b4'}}>??</span>
      </div>
    )
  }
...
export default function ProspectPageClient({ prospectId }: { prospectId: string }) {
  ...
  if (!prospect) return <div className="p-8 text-center" style={{color:'#5c554e'}}>Prospect not found.</div>
        ...
        <div className="flex gap-5 flex-wrap items-start">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              {/* OVR — commissioner only */}
              {isCommissioner ? (
                <div className="flex flex-col items-center justify-center rounded-xl p-3 min-w-[60px]"
                     style={{background:'#fef3c7',border:'1px solid #b4530944'}}>
                  <span className="text-2xl font-black" style={{color:'#b45309'}}>{p.overall}</span>
                  <span className="text-xs font-semibold" style={{color:'#b45309'}}>OVR</span>
                </div>
              ) : (
                <div className="flex flex-col items-center justify-center rounded-xl p-3 min-w-[60px]"
                     style={{background:'#f0ece5',border:'1px solid #d4cdc5'}}>
                  <span className="text-2xl font-black" style={{color:'#c8c0b4'}}>?</span>
                  <span className="text-xs font-semibold" style={{color:'#c8c0b4'}}>OVR</span>
                </div>
              )}
            </div>
      ...
      {!isCommissioner && myTeamId && (
        <div style={{
          ...
          <div style={{fontSize:12,color:'#5b21b6'}}>
            🔍 Scouted: <strong>{revealedCount}/30</strong> attributes revealed for {p.name}
          </div>
